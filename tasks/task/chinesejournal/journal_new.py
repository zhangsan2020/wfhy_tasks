...
class ChinaJournal():
    ...
    def __init__(self):
        ...
        # self.searchinfo = {'mode': '参考文献', 'keywords': '手术', 'max_year': 2022, 'min_year': 2009, 'max_page': 300,'page_size': 50}
    ...
    def get_search_info(self):
        ...
        # for search_info in search_info_wp:
        #     if search_info['have_start'] == 0:
        #         print('选择出待抓取的搜索信息为: ', search_info)
        #         self.search_info = search_info
        #     elif search_info['have_end'] == 0:
        #         print('当前 search_info 之前已经抓去过,但未结束!! 基本信息为: {}'.format(search_info))
        #     elif search_info['have_end'] == 1:
        #         print('当前 search_info 已成功抓取完成, 基本信息为: {}'.format(search_info))
        #     else:
        #         print('未获取到 待搜索数据, 退出程序!!!')
        #         exit()
        with open(self.tasks_path,'r',encoding='utf-8') as f:
            json_data = json.loads(f.read())
            f.close()
        ...
        if new_list:
            ...
            self.update_task(self.search_info,have_start=1)
    ...
    def enterlist(self):
        ...
        data = {
            "key": "Y={}".format(self.searchinfo['keywords']),
            "isNoteHistory": "1",
            "isLog": "1",
            "indexKey": "{}".format(self.searchinfo['keywords']),
            "indexIdentifier": "Y"
        }
    ...
    def get_detail(self, detail_id, article_title):
        ...
        try:
            ...
            wait_times = random.randint(2,10)
            print('详情页下载失败, 暂停 {} 秒钟, 重新开始下载'.format(wait_times))
    ...
    def save(self):
        ...
        self.item['origin_moudle'] = '{}_{}_{}'.format(self.searchinfo['mode'],self.searchinfo['keywords'],self.cur_year)
    ...
    def run(self):
        ...
        for year in range(self.searchinfo['max_year'], self.searchinfo['min_year'], -1):
            ...
            for page in range(1, self.searchinfo['max_page']):
                print('当前是 {} 年, {}, {}, 第 {} 页'.format(year,self.searchinfo['mode'],self.searchinfo['keywords'],page))
                print('基本信息为: {}'.format(str(self.searchinfo)))
