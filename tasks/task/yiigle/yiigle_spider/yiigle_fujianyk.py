...
class Yiigle():
    ...
    def get_search_info(self):
        ...
        for base_info in json_data:
            if base_info['owning_account'] == self.owning_account and base_info['have_end'] != 1 and base_info['is_running'] != 1  and base_info['mode'] == self.mode:
    ...
    def get_page_list(self):
        ...
        for i in range(self.retry_down_list_max):
            try:
                ...
                print('请求列表页第 {} 次 出现问题, 暂停5~10min, 再次发起请求!!'.format(i))
                time.sleep(random.uniform(300, 600))
            if i >= 5:
                print('请求列表页第 {} 次 出现问题, 休息30~40分钟重新登录并发起请求'.format(i))
                time.sleep(random.uniform(900, 2400))
                self.login_again()
