...
from .get_chaoxing_moudle_new import ChaoXingMoudle
...
class ChaoXing():
    ...
    def __init__(self):
        ...
        self.domain = 'qikan.chaoxing.j.hknsspj.cn'
        self.cx_get_moudles = ChaoXingMoudle(self.owning_account,self.domain)
        self.tasks_path = './task/chaoxing/chaoxing_config_second_topic.json'
    ...
    def get_page_num(self):
        ...
        total_num = self.second_total
        new_page_num = self.second_total // self.search_info['page_size'] + 1
        if new_page_num > 15:
            new_page_num = 15
        print('检测到当前模块总条数为: {},显示页数为: {}'.format(total_num,new_page_num))
        if sql_page_data == 0:
    ...
    def get_detail(self):
        ...
        headers = {
                ...
                "Referer": "http://{}/searchjour?sw={}&topsearch=6&size={}".format(self.domain,quote(self.search_info['keywords']),self.search_info['page_size']),
                "Accept-Encoding": "gzip, deflate",
                "Accept-Language": "zh-CN,zh;q=0.9"
            }
        for i in range(3):
            print('这是detail_url',self.item['detail_url'])
            # print(self.session.cookies.get_dict())
            # add_cookies = {
            #     'ishide':'true',
            #     'historySearchWord':'%E5%8C%BB%E5%AD%A6%2C3%2C0301%3B%E5%8C%BB%E9%99%A2%2C3%2C0228'
            # }
            # self.session.cookies.update(add_cookies)
            # print('这是最新cookies: ',self.session.cookies.get_dict())
            try:
            ...
            if res.status_code == 200 and '阅读全文' in res.text:
                ...
                # 译文通信作者信息
                trans_authors = html.xpath('//p[@data-meta-name="译文通信作者"]/text()')
                if trans_authors:
                    trans_authors = trans_authors[0].strip()
                else:
                    print('当前没有译文通信作者简介!! 连接地址为: {}'.format(self.item['detail_url']))
                    trans_authors = ''
                # 第一作者简介信息
                ...
                self.item['doctor_info'] = (trans_authors + author_info + communi_author).replace(' ','').replace('\n','').replace('\r\n','')
    ...
    def get_page_list(self):
        ...
        list_url = "http://{}/searchjour?sw={}&strkey={}&stryear={}&strclassfy=16_-1&topsearch=6&adv=JN{}&aorp=a&nosim=1&size={}&isort=3&x=0_491&pages={}".format(self.domain,
            quote(self.search_info['keywords']), self.second_value,self.years_match[self.cur_year], quote_param,
            self.search_info['page_size'],self.cur_page)
        ...
        header_list = {
            ...
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/109.0.0.0 Safari/537.36",
            "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
            "Referer": "http://{}/searchjour?sw={}&strkey={}&stryear={}&strclassfy=16_-1&topsearch=6&adv=JN{}&aorp=a&nosim=1&size={}&isort=3&x=0_491".format(self.domain,
                quote(self.search_info['keywords']), self.second_value,self.years_match[self.cur_year], quote_param,
                self.search_info['page_size']),
            "Accept-Encoding": "gzip, deflate",
            "Accept-Language": "zh-CN,zh;q=0.9"
        }
            ...
            if res.status_code == 200 and '发表时间' in res.text:
                ...
                if len(trs) > 0:
                    for tr in trs:
                        ...
                        print('***' * 20)
                        if tr.xpath('.//td/a[@class="down"]'):
                            tds = tr.xpath('./td')
                            self.item['owning_account'] = self.owning_account
                            self.item['title'] = tds[1].xpath('./a/@title')[0].strip()
                            detail_url = tds[1].xpath('./a/@href')
                            if detail_url:
                                self.item['detail_url'] = urljoin('http://{}/'.format(self.domain),detail_url[0])
                                ...
                                if status == 1:
                                    ...
                                    if tds[8].xpath('./span/text()')[0].strip():
                                        self.item['read_num'] = tds[8].xpath('./span/text()')[0].strip()
                                    else:
                                        self.item['read_num'] = 0
                                    self.item['cur_page'] = self.cur_page
                                    self.item['cur_year'] = self.cur_year

                                    self.item['origin_moudle'] = '{}_{}_{}_关键词_{}'.format(self.search_info['platform'],
                            ...
                            print('当前项目没有pdf下载, 大概率为超星知识图谱, 不抓取, 跳过!!')
                            continue
                        print('***' * 20)
            ...
            try:
                ...
                for search_info in self.search_infos:
                    ...
                    for year in range(self.search_info['max_year'], self.search_info['min_year'], -1):
                        self.cur_year = year
                        # 获取请求session 以及对应的二级模块!!
                        self.session,second_moudles = self.cx_get_moudles.get_moudles(search_info,year)
                        for second_data in second_moudles.values():
                            self.second_name = second_data['name']
                            self.second_value = second_data['value']
                            self.second_total = second_data['total_num']
                            self.item['origin_moudle'] = '{}_{}_{}_关键词_{}'.format(self.search_info['platform'],
                                                                               self.search_info['keywords'],
                                                                               self.cur_year,self.second_name)
                                ...
                                while self.cur_page > min_page:
                                    print(
                                        '当前是{} {} {} {}年{} 第 {} 页数据'.format(self.owning_account, self.search_info['platform'],
                                                                          ...
                                                                          self.second_name,self.cur_page))
