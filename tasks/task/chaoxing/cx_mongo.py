...
class CxMongo(MongoStore):
    ...
    def find_page_num(self, owning_account, origin_moudle):
        ...
        page_max_data = [page_num['cur_page'] for page_num in self.col.find(
            {'$and': [{'owning_account': owning_account}, {'origin_moudle': origin_moudle}]}).sort("cur_page",
                                                                                                   -1).limit(1)]
        ...
        if page_max_data and page_min_data:
            #     至少有一条数据且能获取到最大最小值
            cur_moudle_sql_total = self.col.count_documents({'$and': [{'origin_moudle': origin_moudle}, {'owning_account': owning_account}]})
