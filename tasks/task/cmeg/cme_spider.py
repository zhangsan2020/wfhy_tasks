...
class CmeSpider():
    ...
    def __init__(self):
        ...
        self.page_size = 150
        self.cme_config = {'page_size': 150, 'max_year': 2023, 'min_year': 2014}
    ...
    def get_mongo_max_pagenum(self):
        ...
        res = self.cme_mongo_all.find_max_page(self.cur_year)
        return res
            ...
            if res.status_code == 200:
                ...
                if trs:
                    for tr in trs:
                        ...
                        self.item['project_name'] = tds[1].xpath('./text()')[0].strip()
                        self.item['unit'] = tds[2].xpath('./text()')[0].strip()
                        self.item['name'] = tds[3].xpath('./text()')[0].strip()
                        self.item['phone'] = tds[4].xpath('./text()')[0].strip()
                        if tds[9].xpath('./text()'):
                            self.item['student_num'] = tds[9].xpath('./text()')[0].strip()
                        else:
    ...
    def save(self):
        ...
        if self.item['phone'] and re.findall('(1[35678]\d{9})', self.item['phone']):
            status = self.cme_redis.set_item('cme_phones', self.item['phone'])
            if status == 1:
