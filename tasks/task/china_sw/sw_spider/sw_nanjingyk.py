...
class ChinaSw():
    ...
    def __init__(self):
        ...
        self.spider_at = 'win'
        self.owning_account = '6849_中国生物_东单校园网'
        ...
        self.tasks_path = './task/china_sw/chinasw_title_config.json'
        ...
        self.domain = 'www-sinomed-ac-cn.beijingxiehe.tsg211.com'
    ...
    def get_page_num(self, year):
        ...
        '''
        list_url = 'http://{}/zh/basicSearch.do'.format(self.domain)
        data = {
                "sql": "TI/1='{}' AND DP_YEAR/1='{}'".format(self.search_info['keywords'],self.cur_year),
                "dbtype": "zh",
                "pageNo": "1",
                "pageSize": "{}".format(self.search_info['page_size']),
                "ajax": "items",
                "searchword": "(\"{}\"[{}:智能]) AND (\"{}\"[时间]) ".format(self.search_info['keywords'],self.search_info['platform'],self.cur_year),
                "cmode": "tl",
                "orderBy": "LIFO",
                "flag": "true",
                "selected": "undefined"
            }
        # http://www-sinomed-ac-cn.beijingxiehe.tsg211.com/crossSearch.do?searchwordwm=手术标题:智能
        header_list = {
                    ...
                    "Referer": "http://{}/zh/basicSearch.do?searchwordwm={}{}:{}".format(self.domain,quote(self.search_info['keywords']),quote(self.search_info['platform']),quote('智能')),
                    "Accept-Encoding": "gzip, deflate",
                    "Accept-Language": "zh-CN,zh;q=0.9"
                }

        print('这是cookies: ',self.session.cookies.get_dict())
        print('这是data: ',data)
        print('这是header: ',header_list)
    ...
    def get_page_list(self):
        ...
        for i in range(self.retry_down_list_max):
            ...
            # list_url_before = 'http://www-sinomed-ac-cn.beijingxiehe.tsg211.com/index.do?method=stat'
            # headers = {
            ...
            #     "X-Requested-With": "XMLHttpRequest",
            #     "Origin": "http://www-sinomed-ac-cn.beijingxiehe.tsg211.com",
            #     "Referer": "http://www-sinomed-ac-cn.beijingxiehe.tsg211.com/index.jsp",
            #     "Accept-Encoding": "gzip, deflate",
            #     "Accept-Language": "zh-CN,zh;q=0.9",
            # }
            # try:
            #     self.session.post(list_url_before,headers=headers, timeout=(20, 30))
            # except Exception as e:
            #     print('请求列表页出现异常, 休息一下, 重新获取列表页数据!!')
            #     time.sleep(random.uniform(30, 60))
            #     continue
            # list_url = 'http://www-sinomed-ac-cn.beijingxiehe.tsg211.com/crossSearch.do'
            # data = {
            #     "sql": "TI/1='{}' AND DP_YEAR/1='{}'".format(self.search_info['keywords'], self.cur_year),
            #     "dbs": "zh,en,lw,kp",
            #     "db": "zh",
            ...
            #     "pageSize": "{}".format(self.search_info['page_size']),
            #     "ajax": "items",
            #     "searchword": "(\"{}\"[{}:智能]) AND (\"{}\"[时间]) ".format(self.search_info['keywords'],
            ...
            #     "Content-Type": "application/x-www-form-urlencoded",
            #     "Origin": "http://www-sinomed-ac-cn.beijingxiehe.tsg211.com",
            #     "Referer": "http://www-sinomed-ac-cn.beijingxiehe.tsg211.com/crossSearch.do?searchwordwm={}{}:{}".format(
            #         quote(self.search_info['keywords']), quote(self.search_info['platform']), quote('智能')),
            #     "Accept-Encoding": "gzip, deflate",
            #     "Accept-Language": "zh-CN,zh;q=0.9"
            # }
            ...
            header_list = {
                ...
                "Content-Type": "application/x-www-form-urlencoded",
                "Origin": "http://{}".format(self.domain),
                "Referer": "http://{}/zh/basicSearch.do?searchwordwm={}{}:{}".format(self.domain, quote(
                    self.search_info['keywords']), quote(self.search_info['platform']), quote('智能')),
                "Accept-Encoding": "gzip, deflate",
                "Accept-Language": "zh-CN,zh;q=0.9"
            }
            try:
                            ...
                            if pdf_flag and 'pdf.jpg' in pdf_flag[0].strip():
                                ...
                                if status == 1:
                                    ...
                                    try:
                                        ...
                                        self.item['origin'] = '不详'
                                    # http://www-sinomed-ac-cn.beijingxiehe.tsg211.com/zh/detail.do?ui=2023103671
                                    self.item['detail_url'] = 'http://www-sinomed-ac-cn.beijingxiehe.tsg211.com' + \
                                                              div.xpath('./h2/a/@href')[0].strip()
    ...
    def down_pdf(self, file_path):
        ...
        for i in range(self.retry_down_pdf_max):
            headers_pdf = {
                ...
                "Referer": "http://www-sinomed-ac-cn.beijingxiehe.tsg211.com/",
                "Accept-Encoding": "gzip, deflate",
                "Accept-Language": "zh-CN,zh;q=0.9",
            }
            print('开始下载pdf文件...')
            ...
            if r.status_code == 200:
                ...
                if self.down_pdffail_num >= 10:
                    print('pdf下载出错超过10次, 等待3~5分钟, 重新开始下载数据!!')
                    time.sleep(random.uniform(180, 300))
                    self.down_pdffail_num = 0
        else:
    ...
    def get_pdf_url(self, ti, id_):
        ...
        url = 'http://www-sinomed-ac-cn.beijingxiehe.tsg211.com/zh/relatedLink.do?method=pdfLink&sf_request_type=ajax'
        data = {
            "id": id_,
            "ti": ti,
            "db": "zh_pdf"
        ...
        headers = {
            ...
            "Content-Type": "application/x-www-form-urlencoded",
            "Origin": "http://www-sinomed-ac-cn.beijingxiehe.tsg211.com",
            # "Referer": "http://www-sinomed-ac-cn.beijingxiehe.tsg211.com/crossSearch.do?searchwordwm={}".format(quote('{}  {}时间'.format(self.search_info['keywords'],self.cur_year))),
            "Accept-Encoding": "gzip, deflate",
            "Accept-Language": "zh-CN,zh;q=0.9",
            # "Cookie": "JSESSIONID=F5D9352138AB32221F3D91B7F1C27639; com.trs.idm.coSessionId=F5D9352138AB32221F3D91B7F1C27639; ddhgguuy_session=1roih2gvj2beciugingl121it0; uuid=1675837375325.31; history_cross=history_52067792-1; refreshedTimestamp=1675837986787"
        }
    ...
    def run(self):
        ...
        for i in range(10):
            ...
            try:
                ...
                time.sleep(random.uniform(60, 120))
                self.update_task(self.search_info, have_start=1, is_running=0, cur_year=self.cur_year)
                time.sleep(random.uniform(5,10))
                print('发生了错误, 即将根据循环再次进入!!')
