...
class WeiPu():
    ...
    def __init__(self):
        ...
        self.domain = 'vp.vip.tsg211.com'
        self.identify = CurIdentify()
        self.redis_wp = WpRedis()
        self.redis_all_phones = WpRedis()
        self.wp_mongo_commit = WpMongo('wfhy_commit', 'weipu_commit')
        self.wp_mongo_all = WpMongo('wfhy_update', 'weipu_all')
        # 当前已表格模式进行数据抓取, 模式每页显示50条数据, 没办法重置
        # self.search_info = {'platform': '参考文献', 'keywords': '手术', 'max_year': 2022, 'min_year': 2009, 'max_page': 300,'page_size': 50}
    ...
    def update_task(self, condition, **kwargs):
        ...
        for index, search_info in enumerate(json_data):
            if search_info['platform'] == platform and search_info['keywords'] == keywords and search_info[
    ...
    def check_task_info(self,search_info):
        ...
        for task_info in json_data:
            task_info_compare = '{}_{}_{}_{}'.format(task_info['platform'],task_info['keywords'],task_info['max_year'],task_info['min_year'])
            search_info_compare = '{}_{}_{}_{}'.format(search_info['platform'],search_info['keywords'],search_info['max_year'],search_info['min_year'])
            if task_info['platform'] == self.platform:
    ...
    def get_search_info(self):
        '''
        获取执行任务的基本信息, 执行任务可以存在多个, 且排队做执行. 选择执行任务时, 任务队列需满足 owning_account(所属账号),have_end(未抓取过),is_running(没在执行),spider_at(抓取平台 win/linux), platform(抓取的方向 全库/病例), 符合这几个条件则进入任务队列排队执行
        ...
        for base_info in json_data:
            if  base_info['platform'] == self.platform and base_info['have_end'] != 1 and base_info['is_running'] != 1:
    ...
    def request_list(self):
        list_url = 'http://{}/Search/SearchList'.format(self.domain)
        header_list = {
            ...
            "Origin": "http://vp.vip.tsg211.com",
            "Referer": "http://vp.vip.tsg211.com/Qikan/Search/Index?from=index",
            "Accept-Encoding": "gzip, deflate",
            "Accept-Language": "zh-CN,zh;q=0.9,en;q=0.8",
            # "Cookie": "ASP.NET_SessionId=iazkeynbnmvkhu2hffog1pgg; Md5Image=ff74847a13; LIBUSERIDCOOKIE=98; LIBUSERCOOKIE=Oosn4ui+3LIhKzbcJOalvWEpNIfNM241N2+GE2oKLqMG0pZvgPNxwxapyte7pqiL1AnLDVxPZIdwrEKcRtGxsNqvsRakovjr1mIBRnYkG2TX60E+31+UEy5QCVKi4yGhCkUDoW7WHNyCoFN9Z6vnpphTPH7KMCPzRuGlXKbUblraRuaDw/7iHdxPm52WP/XlHGlaTXvZaOp5jG8IhX4E4Cywu0US7a7847AJAY4hRdYrL6M2rQdAsA=="
        }
        searchParamModel='{{"ObjectType":1,"SearchKeyList":[],"SearchExpression":null,"BeginYear":null,"EndYear":null,"UpdateTimeType":null,"JournalRange":null,"DomainRange":null,"ClusterFilter":"YY=2023#2023","ClusterLimit":0,"ClusterUseType":"Article","UrlParam":"S={keywords}","Sort":"2","SortField":null,"UserID":"2","PageNum":1,"PageSize":100,"SType":"","StrIds":"","IsRefOrBy":0,"ShowRules":"  年份={cur_year}   AND   {platform}={keywords}  ","IsNoteHistory":0,"AdvShowTitle":null,"ObjectId":null,"ObjectSearchType":0,"ChineseEnglishExtend":0,"SynonymExtend":0,"ShowTotalCount":10000,"AdvTabGuid":""}}'.format(keywords=self.search_info['keywords'],cur_year=self.cur_year,platform=self.search_info['platform'])
        # data = {'searchParamModel':searchParamModel}
        data = 'searchParamModel={}'.format(quote(searchParamModel))
        print('开始发起get_page_num列表页请求, 目的是获取模块页码数据!!')
        for i in range(self.retry_down_list_max):
            ...
            if res.status_code == 200 and '请重新进入' not in res.text:
                ...
                total_value = html.xpath('//input[@id="hidShowTotalCount"]/@value')
                page_num_data = html.xpath('//a[@class="layui-laypage-last"][1]/@data-page')
                if total_value and page_num_data:
                    total = int(total_value[0])
                    self.ShowTotalCount = total
                    new_page_num = int(page_num_data[0])
                    print('当前模块含有文章 {} 页, 共 {} 条'.format(new_page_num, total))
                else:
                    print('get_page_num 没有获取到文章总数, 重新获取!!')
                    continue
                return total,new_page_num
            elif '抱歉，暂无数据' in res.text:
                print('当前模块没有数据,进入下一模块')
                return 'no_data'
            elif '重新进入' in res.text:
                print('列表页请求时发现 重新进入字眼, 重新登录获取数据!!')
    ...
    def find_mongo_page(self):
        '''
        根据账号和origin_module信息锁定待抓取当前年之前已经抓取过保存在数据库中的最大的页面
        :return:
        '''

        sql_page = self.wp_mongo_all.find_wp_page_num(self.origin_module)
    ...
    def get_page_list(self):
        ...
        searchParamModel = '{{"ObjectType":1,"SearchKeyList":[],"SearchExpression":null,"BeginYear":null,"EndYear":null,"UpdateTimeType":null,"JournalRange":null,"DomainRange":null,"ClusterFilter":"YY={cur_year}#{cur_year}","ClusterLimit":0,"ClusterUseType":"Article","UrlParam":"S={keywords}","Sort":"2","SortField":null,"UserID":"2","PageNum":{cur_page},"PageSize":{page_size},"SType":"","StrIds":"","IsRefOrBy":0,"ShowRules":"  年份={cur_year}   AND   {platform}={keywords}  ","IsNoteHistory":0,"AdvShowTitle":null,"ObjectId":null,"ObjectSearchType":0,"ChineseEnglishExtend":0,"SynonymExtend":0,"ShowTotalCount":{ShowTotalCount},"AdvTabGuid":""}}'.format(
            keywords=self.search_info['keywords'], cur_page=self.cur_page, page_size=self.search_info['page_size'], cur_year=self.cur_year, platform=self.search_info['platform'],ShowTotalCount=self.ShowTotalCount)
        # data = {'searchParamModel':searchParamModel}
        data = 'searchParamModel={}'.format(quote(searchParamModel))
        print('这是data数据; ',data)
        list_url = 'http://{}/Search/SearchList'.format(self.domain)
            ...
            if res.status_code == 200 and '请重新进入' not in res.text:
                ...
                for dl in dls:
                    ...
                    if '下载PDF' in down_flag:
                        ...
                        # self.item['file_name'] = '{}_{}.pdf'.format(name,articleid)
                        self.item['file_name'] = re.sub('[’!"#$%\'()*+,/:;<=>?@，。?★、…【】《》？“”‘’！[\\]^`{|}~\s]+', "",name) + '_' + articleid + '.pdf'
                        print('文件名为: {}'.format(self.item['file_name']))
                        filter_status = self.redis_wp.set_item('weipu',self.item['file_name'])
                        if filter_status == 1:
                            self.have_spider_num = 0
                            # http://vp.vip.tsg211.com/Qikan/Article/Detail?id=7108873433
                            self.item['detail_url'] = urljoin('http://'+self.domain,dl.xpath('./dt[1]/a/@href')[0].strip())
                            refer_num = dl.xpath('./dt[1]/span[@class="cited"]/a/text()')
                            if refer_num:
                                self.item['reference_num'] = refer_num[0]
                            else:
                                self.item['reference_num'] = 0
                            author_datas = dl.xpath('.//span[@class="author"][1]/span[position()>1]')
                            if author_datas:
                                self.item['authors'] = ';'.join(
                                    [author_data.xpath('./a/@title')[0] for author_data in author_datas if
                                     len(author_data.xpath('./a/@title')) > 0])
                            else:
                                self.item['authors'] = ''
                            origin_from_data = dl.xpath('.//span[@class="from"][1]/a/@title')
                            if origin_from_data:
                                self.item['origin_from'] = dl.xpath('.//span[@class="from"][1]/a/@title')[0].strip()
                            else:
                                self.item['origin_from'] = ''
                            self.item['article_keyword'] = ';'.join(dl.xpath('.//span[@class="subject"][1]/span/a/@title'))
                            self.item['owning_account'] = self.owning_account
                            self.item['cur_page'] = self.cur_page
                            self.item['cur_year'] = self.cur_year
                            self.item['origin_moudle'] = self.origin_module
                            # a[contains(text(),"电子账户")]
                            # pdf_params_data = dl.xpath('.//div[@class="article-source"]/a[contains(text(),"下载PDF")]')
                            pdf_params_tag = etree.tostring(
                                dl.xpath('.//div[@class="article-source"][1]')[0]).decode().strip()
                            ...
                            if pdf_params_data and pdf_params_data[0]:
                                ...
                                pdf_params = 'id={}&info={}&ts={}'.format(pdf_params_data[0][0], pdf_params_data[0][1],time_)
                                # print(pdf_params)
                                pdf_url_status = self.get_pdf_url(pdf_params)
                                if pdf_url_status:
                                    print('成功获取到PDF连接地址!!')
                                    file_path = '{}/{}'.format(self.dir_pdfs,self.item['file_name'])
                                    ...
                                    print('连续三次没有获取到 pdf_url连接地址,跳过当前项目!!')
                                    continue
                            else:
                                print('获取PDF文件请求参数时出现问题, 请仔细查看!!')
                                self.item['identify_status'] = 'pdf请求参数异常'
                                self.save()
                                continue
                            #     这里加上参数获取错误字段, 方便后期重构
                        else:
                ...
                print('列表页第 {} 页下载失败,暂停3~5min, 重新下载!!'.format(self.cur_page))
    ...
    def get_pdf_url(self,pdf_params):
        ...
        headers = {
            ...
            "Referer": "http://{}/Qikan/Search/Index?from=index".format(self.domain),
            "Accept-Encoding": "gzip, deflate",
            "Accept-Language": "zh-CN,zh;q=0.9,en;q=0.8"
        }
        url = 'http://{}/Qikan/Article/ArticleDown'.format(self.domain)
        for i in range(3):
            try:
                res = self.session.post(url,data=pdf_params,headers=headers,timeout=(20,30))
                if res.status_code == 200 and '{"url":"' in res.text:
                    self.item['pdf_url'] = res.json()['url']
                    print('成功获取到pdf连接地址: {}'.format(self.item['pdf_url']))
                    return 1
                else:
                    print('获取pdf链接出错,重新请求获取!! 这是返回的数据: {}'.format(res.text))
            except Exception as e:
                print('下载pdf文件第 {} 次链接读取失败或请求出错,信息为: {},不打紧再来!!'.format(i,repr(e)))
    ...
    def down_pdf(self,file_path):
        ...
        for i in range(self.retry_down_pdf_max):
            print('开始下载pdf文件...')
            try:
                res = self.session.get(self.item['pdf_url'], headers=headers, timeout=(20, 30))
                time.sleep(random.uniform(2, 3))
                if res.status_code == 200 and '重新进入' not in res.text:
                    ...
                    self.down_pdffail_num = 0
                    break
                elif '重新进入' not in res.text:
                    print('出现重新进入字眼,重新登录之后再次获取PDF文件!')
                    time.sleep(random.uniform(3,5))
                    self.login_again()
                else:
                    print('pdf文件下载失败第 {} 次, url 为 {} 信息为: {}, 等待一下重新下载'.format(i, self.item['pdf_url'], self.item['file_name']))
                    time.sleep(random.uniform(3, 5))
            except:
        ...
        if self.down_pdffail_num >= 10:
            print('连续 10 个pdf文件下载失败, 可能遇到反爬, 休息2~3分钟后, 向后跳 {} 页, 继续抓取!!'.format(self.turn_page_num))
            time.sleep(random.uniform(120, 180))
            self.login_again()
            # self.cur_page = self.turn_page_num
            # self.down_pdffail_num = 0
    ...
    def create_index(self):
        md5_data = '{}_{}'.format(str(int(time.time()*1000)),self.item['file_name']).encode()
        _id = hashlib.md5(md5_data).hexdigest()
        print(_id)
        return _id
    ...
    def save(self):
        ...
        self.item['_id'] = self.create_index()
        if 'phones' in self.item and self.item['phones']:
            if len(self.item['phones']) > 1:
                ...
                for phone in self.item['phones']:
                    ...
                    phones['phones'] = phone
                    self.wp_mongo_all.insert(phones)
                    ...
                    if duplicate_status == 1:
                        ...
                        self.wp_mongo_commit.insert(phones)
                ...
                self.wp_mongo_all.insert(self.item)
                # 去重状态,目的是去除重复的手机号, 存储在commit中的一定是之前没有见过的新手机号,最终给到销售
            ...
            print('发现新数据, 但没有手机号, 将手机号重置为空, 放入weipu_all中')
            self.item['phones'] = ''
            self.wp_mongo_all.insert(self.item)
        self.item.clear()
            ...
            for search_info in self.get_search_info():
                if search_info:
                    ...
                    for year in range(self.search_info['max_year'], self.search_info['min_year'], -1):
                        ...
                        self.origin_module = '{}_{}_{}'.format(self.search_info['platform'],
                                                                           self.search_info['keywords'],
                                                                           self.cur_year)
                        self.item['origin_module'] = self.origin_module

                        print('当前是 {} 年, 基本信息为: {}'.format(self.cur_year, str(self.search_info)))
                        # module_end_year_data = '{}'.format(self.item['origin_module'],
                        #                                          self.main_task_info['keywords'], self.cur_year)
                        year_spider_status = self.wp_mongo_all.get_module_end_year(self.origin_module)
                        if year_spider_status != 'cur_year_over':
                            ...
                            if data:
                                ...
                                while self.cur_page > min_page:
                                    ...
                                    print('当前是{} {} {} {}年 第 {} 页数据'.format(self.owning_account, self.search_info['platform'],
                        ...
                        self.wp_mongo_all.insert_module_end_year(self.origin_module)
