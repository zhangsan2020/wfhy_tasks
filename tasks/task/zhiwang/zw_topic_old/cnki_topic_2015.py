...
class ZwSpider():
    ...
    def get_page_items(self, page_num, year, item_data):

        self.queryjson = '{"Platform":"","DBCode":"CFLS","KuaKuCode":"CJFQ,CDMD,CIPD,CCND,BDZK,CISD,SNAD,CCJD,GXDB_SECTION,CJFN,CCVD","QNode":{"QGroup":[{"Key":"Subject","Title":"","Logic":1,"Items":[{"Title":"' + self.searchinfo['model'] + '","Name":"' + self.searchinfo['model_match_data'] + '","Value":"' + item_data['name'] + '","Operate":"%=","BlurType":""}],"ChildItems":[]},{"Key":"SCDBGroup","Title":"","Logic":1,"Items":[],"ChildItems":[{"Key":"3","Title":"","Logic":1,"Items":[{"Key":"' + year + '","Title":"' + year + '","Logic":2,"Name":"年","Operate":"","Value":"' + year + '","ExtendType":0,"ExtendValue":"","Value2":"","BlurType":""}],"ChildItems":[]}]}]}}'
        params = {
            'IsSearch': 'true',
            'QueryJson': self.queryjson,
                ...
                if tr.xpath('./td[@class="name"]/a/@href'):
                    ...
                    try:
                        ...
                        if self.page_items_retry < 3:
                            ...
                            time.sleep(60)
                            self.get_searchsql()
                            page_items = self.get_page_items(page_num, year, item_data)
                        else:
                            self.page_items_retry = 0
                            page_items = ''
    ...
    def get_page_list(self):
        ...
        for year in range(self.max_year,self.min_year,-1):
            ...
            for items in json_data['keywords'].values():
                for index,item_data in enumerate(items):
                    self.searchinfo = {'model':'主题','keywords':item_data['name']}
                    self.get_searchsql()
                    print('当前是 {} 年, 主题模块为: {}, 子集为: {}'.format(year, item_data['search_origin'],item_data['name']))
                    time.sleep(2)
                    have_spider_year = 'have_spider_{}'.format(year)
                    print('这是 have_spider_year: {}'.format(have_spider_year))
                    if not 'have_spider_year' in item_data:
                        print('当前模块未被抓去过, 可放心大胆去抓取!!!')
                        time.sleep(2)
                        for page_num in range(1, 301):
                            print('***' * 20)
                            print('当前是 {} 年, 主题模块为: {}, 子集为: {} 的 第 {} 页'.format(year, item_data['search_origin'],item_data['name'],page_num))
                            print('***' * 20)
                        ...
                        json_data['keywords'][item_data['search_origin']][index][have_spider_year] = spider_date
                        new_data = json.dumps(json_data, ensure_ascii=False)
                        with open(self.ori_jsondata_url, 'w', encoding='utf-8') as f:
                            f.write(new_data)
                        print('开始进入下一模块...., 请稍后!!')
                    else:
                        print('{} 模块已经抓去过, 开始进入下一模块.'.format(item_data['name']))
                        time.sleep(2)
        ...
        for item in page_item_data:
            ...
            if duplicate_status == 1:
                ...
                if res.status_code == 200 and 'pdfDown' in res.text:
                    ...
                    time.sleep(random.uniform(1,3))
                    if self.havepdfdown > 20:
                        print('连续 20 次获取详情页连接地址出现问题, 暂停90秒, 重新登录获取!')
                        time.sleep(180)
                        self.get_searchsql()
                        self.havepdfdown = 0
            else:
    ...
    def save(self, item):
        ...
        item['origin_moudle'] = '主题_搜索_{}'.format(self.year)
