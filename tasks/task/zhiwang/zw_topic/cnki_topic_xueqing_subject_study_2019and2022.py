...
class ZwSpider():
    ...
    def __init__(self):
        ...
        self.ori_jsondata_url = './task/zhiwang/moudle_data/old_主题_血清_学科_研究层次.json'
        ...
        # self.searchinfo = {'model': '参考文献', 'keywords': '肝炎'}
        # self.searchinfo = {'model': '参考文献', 'keywords': '病'}
        # self.searchinfo = {'model': '作者单位', 'keywords': '西医'}
        # self.searchinfo = {'model': '作者单位', 'keywords': '中医'}
        # self.searchinfo = {'model': '作者单位', 'keywords': '护理'}
        self.searchinfo = {'model': '主题', 'keywords': '血清'}
    ...
    def get_page_items(self, page_num, year, subject_info, item_info):
        ...
        self.queryjson = '{"Platform":"","DBCode":"CFLS","KuaKuCode":"CJFQ,CDMD,CIPD,CCND,BDZK,CISD,SNAD,CCJD,GXDB_SECTION,CJFN,CCVD","QNode":{"QGroup":[{"Key":"Subject","Title":"","Logic":1,"Items":[{"Title":"' + \
                         ...
                         self.searchinfo[
                             'keywords'] + '","Operate":"%"}],"ChildItems":[]},{"Key":"MutiGroup","Title":"","Logic":1,"Items":[],"ChildItems":[{"Key":"2","Title":"","Logic":1,"Items":[{"Key":"' + \
                ...
                for item_info in moudle_data['study_level'].values():
                    ...
                    if not 'have_spider' in json_data[year][index]['study_level'][little_key]:
                        ...
                        for page_num in range(1, 301):
                            print('***' * 20)
                            print(
                                '当前是 {} 年, 学科为: {}, 标题为: {} 中的第 {} 页数据'.format(year, str(subject_info['subject_name']),
                                                                               little_key, page_num))
                            print('***' * 20)
        ...
        for item in page_item_data:
            ...
            if duplicate_status == 1:
                ...
                if res.status_code == 200 and ('分页下载' not in res.text) and 'pdfDown' in res.text:
                    ...
                    self.havepdfdown += 1
                    print('连续第 {} 次获取详情页连接地址出现问题, 状态不为200 或者页面内不含有pdfdown! 放慢下速度... url地址为: {}'.format(self.havepdfdown,
                                                                                                       res.url))
                    self.log.info(
                        '连续第 {} 次获取详情页连接地址出现问题, 状态不为200 或者页面内不含有pdfdown! 放慢下速度... url地址为: {}'.format(self.havepdfdown,
                                                                                                     res.url))
                    time.sleep(random.uniform(1, 3))
