...
class ZwSpider():
    ...
    def get_page_list(self):
        ...
        # for year in years:
        year = '2022'
        with open('./task/zhiwang/old_unit_hospital_sub_study.json', 'r', encoding='utf-8') as f:
            data = f.read()
        moudle_datas = json.loads(data)[year]
        for moudle_data in moudle_datas:
            subject_info = moudle_data['subject_info']
            for study_info in moudle_data['study_level'].values():
                print('这是study_info',study_info)
                print('这是subject_info',moudle_data['subject_info'])
                for page_num in range(1,301):
                    print('***'*20)
                    print('当前是 {} 年, 学科为: {}, 研究层次为: {} 中的第 {} 页数据'.format(year,str(subject_info['subject_name']),str(study_info['study_level_name']),page_num))
                    print('***'*20)
