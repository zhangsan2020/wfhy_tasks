...
class ZwSpider():
    ...
    def __init__(self):
        ...
        self.ori_jsondata_url = './task/zhiwang/moudle_data/old_篇名_细胞_年_学科_主题.json'
    ...
    def request_searchsql(self):
        ...
        for i in range(5):
            try:
                res = self.session.post(self.list_url, data=data, headers=self.headers_list, timeout=(15, 20))
                time.sleep(random.uniform(0, 0.5))
                break
            except:
                time.sleep(60)
    ...
    def check_code(self):
        ...
        # with open('code_text.txt','a',encoding='utf-8') as f:
        #     f.write(res.text)
        if '参数错误' in res.text:
            print('临时验证码识别时, 参数错误!!')
        else:
            ...
            if duplicate_status == 1:
                ...
                if res.status_code == 200 and ('分页下载' not in res.text) and 'pdfDown' in res.text:
                    ...
                    print('连续第 {} 次获取详情页连接地址出现问题, 状态不为200 或者页面内不含有pdfdown! 放慢下速度... url地址为: {}'.format(self.havepdfdown,
                                                                                                   res.url))
                    with open('man_reason.html', 'w') as f:
                        f.write(res.text)
                        f.close()
                    print(res.text)
                    exit()
