...
from .zw_common import user_info_re, headers_list, choice_moudle
...
class ZwSpider():
    ...
    def get_choice_item(self):
        ...
        # group_datas = {}
        self.group_datas = {'subjects':{'info':{}},'topics':[]}
        if res.status_code == 200 and '发表年度' in res.text:
            ...
            self.group_datas['subjects']['field'] = sub_field
            lis_sub = html.xpath('//dd[@tit="学科"]//li')
            # print(lis_sub)
            for li in lis_sub:
                name = li.xpath('./input/@text')[0]
                self.group_datas['subjects']['info'][name] = li.xpath('./input/@value')[0]
                # print(name)

            # group_datas['subject'] = html.xpath('//dd[@tit="学科"]//li/input/@text')
            # 主题
            main_topics = html.xpath('//dd[@tit="主要主题"]//li/input/@text')
            self.group_datas['topics'] = [('主要主题',main_topic) for main_topic in main_topics]
            minor_topics = html.xpath('//dd[@tit="次要主题"]//li/input/@text')
            self.group_datas['topics'].extend([('次要标题',minor_topic) for minor_topic in minor_topics])
        else:
            print('未获取到左侧筛选数据, 退出!!')
    ...
    def get_page_items(self,page_num,year,sub_info,topic_data):

        # 主题
        # "Key":"多层螺旋CT","Title":"多层螺旋CT","Logic":2,"Name":"主要主题","Operate":"","Value":"多层螺旋CT"
        # 学科:
        # "Key":"E076?","Title":"特种医学","Logic":2,"Name":"专题子栏目代码","Operate":"","Value":"E076?"
        # 年:
        # "Key":"2022","Title":"2022","Logic":2,"Name":"年","Operate":"","Value":"2022","ExtendType":0
        # print(topic_data)
        # print(sub_info)
        # print(year)
        # exit()
        self.queryjson = '{"Platform":"","DBCode":"CFLS","KuaKuCode":"CJFQ,CDMD,CIPD,CCND,BDZK,CISD,SNAD,CCJD,GXDB_SECTION,CJFN,CCVD","QNode":{"QGroup":[{"Key":"Subject","Title":"","Logic":1,"Items":[{"Title":"作者单位","Name":"AF","Value":"医院","Operate":"%"}],"ChildItems":[]},{"Key":"SCDBGroup","Title":"","Logic":1,"Items":[],"ChildItems":[{"Key":"1","Title":"","Logic":1,"Items":[{"Key":"'+ topic_data[1] +'","Title":"' + topic_data[1] + '","Logic":2,"Name":"'+ topic_data[0] +'","Operate":"","Value":"' + topic_data[1] + '","ExtendType":0,"ExtendValue":"","Value2":"","BlurType":""}],"ChildItems":[]},{"Key":"2","Title":"","Logic":1,"Items":[{"Key":"'+ sub_info[1] +'?","Title":"' + sub_info[0] + '","Logic":2,"Name":"' + self.group_datas['subjects']['field'] + '","Operate":"","Value":"' + sub_info[1] + '?","ExtendType":14,"ExtendValue":"","Value2":"","BlurType":""}],"ChildItems":[]},{"Key":"3","Title":"","Logic":1,"Items":[{"Key":"' + year + '","Title":"' + year + '","Logic":2,"Name":"年","Operate":"","Value":"' + year + '","ExtendType":0,"ExtendValue":"","Value2":"","BlurType":""}],"ChildItems":[]}]}]}}'
                ...
                if tr.xpath('./td[@class="name"]/a/@href'):
                    ...
                    try:
                        ...
                        if self.page_items_retry < 3:
                            ...
                            time.sleep(60)
                            self.get_searchsql()
                            page_items = self.get_page_items(page_num,year,sub_info,topic_data)
                        else:
                            self.page_items_retry = 0
                            page_items = ''
    ...
    def get_page_list(self):
        ...
        for year in years:
            for subject_info in self.group_datas['subjects']['info'].items():
                for topic_data in self.group_datas['topics']:
                    for page_num in range(1, 301):
                        print('***'*20)
                        print('当前是 {} 年, 学科为: {}, 研究层次为: {} 中的第 {} 页数据'.format(year,str(subject_info),str(topic_data),page_num))
                        print('***'*20)
                        self.log.info('当前是 {} 年, 学科为: {}, 研究层次为: {} 中的第 {} 页数据'.format(year,str(subject_info),str(topic_data),page_num))

                        time.sleep(random.uniform(1, 4))
                        page_items = self.get_page_items(page_num,year,subject_info,topic_data)
