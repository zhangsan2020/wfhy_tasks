...
from task.common.proxy_api import get_ip
...
class ZwSpider():
    ...
    def __init__(self):
        ...
        # 27.158.35.198:16248'
        # self.proxies = {'http': '115.208.82.191:20516', 'https': '115.208.82.191:20516'}
        self.proxies = get_ip()
        print(self.proxies)
        # proxyHost = "115.208.82.191"
        # proxyPort = "20516"
        ...
        self.ori_jsondata_url = './task/zhiwang/moudle_data/old_unit_hospital_sub_origin.json'
    ...
    def login(self):
        ...
        res = self.session.get(imgcode_url, headers=self.headers_init, timeout=(5, 20),proxies=self.proxies)
        ...
        res = self.session.get(url, headers=self.headers_init, timeout=(5, 20),proxies=self.proxies)
    ...
    def get_searchsql(self):
        ...
        try:
            self.session.get(url, headers=self.headers_init, timeout=(5, 20),proxies=self.proxies)
        except:
            time.sleep(60)
            self.session.get(url, headers=self.headers_init, timeout=(5, 20),proxies=self.proxies)
        ...
        try:
            res = self.session.get(field_json_url, headers=header_field, proxies=self.proxies)
        except:
            time.sleep(60)
            res = self.session.get(field_json_url, headers=header_field, proxies=self.proxies)
        ...
        try:
            res = self.session.post(self.list_url, data=data, headers=self.headers_list, timeout=(5, 20), proxies=self.proxies)
            time.sleep(random.uniform(0,0.5))
        except:
            time.sleep(60)
            res = self.session.post(self.list_url, data=data, headers=self.headers_list, timeout=(5, 20), proxies=self.proxies)
    ...
    def get_page_items(self, page_num, year, subject_info, item_info):
        ...
        #                  self.searchinfo['keywords'] + '","Operate":"%"}],"ChildItems":[]},{"Key":"SCDBGroup","Title":"","Logic":1,"Items":[],"ChildItems":[{"Key":"1","Title":"","Logic":1,"Items":[{"Key":"' + \
        ...
        self.queryjson = '{"Platform":"","DBCode":"CFLS","KuaKuCode":"CJFQ,CDMD,CIPD,CCND,BDZK,CISD,SNAD,CCJD,GXDB_SECTION,CJFN,CCVD","QNode":{"QGroup":[{"Key":"Subject","Title":"","Logic":1,"Items":[{"Title":"' + self.searchinfo['model'] + '","Name":"' + self.searchinfo['model_match_data'] + '","Value":"' + self.searchinfo['keywords'] + '","Operate":"%"}],"ChildItems":[]},{"Key":"SCDBGroup","Title":"","Logic":1,"Items":[],"ChildItems":[{"Key":"2","Title":"","Logic":1,"Items":[{"Key":"' + subject_info['value'] + '?","Title":"' + subject_info['subject_name'] + '","Logic":2,"Name":"' + subject_info['field'] + '","Operate":"","Value":"' + subject_info['value'] + '?","ExtendType":14,"ExtendValue":"","Value2":"","BlurType":""}],"ChildItems":[]},{"Key":"3","Title":"","Logic":1,"Items":[{"Key":"' + year + '","Title":"' + year + '","Logic":2,"Name":"年","Operate":"","Value":"' + year + '","ExtendType":0,"ExtendValue":"","Value2":"","BlurType":""}],"ChildItems":[]},{"Key":"6","Title":"","Logic":1,"Items":[{"Key":"'+item_info['value']+'","Title":"' + item_info['origin_name']+ '","Logic":2,"Name":"' + item_info['origin_field'] + '","Operate":"","Value":"' + item_info['value'] + '","ExtendType":14,"ExtendValue":"","Value2":"","BlurType":""}],"ChildItems":[]}]}]}}'
        ...
        try:
            res = self.session.post(self.list_url, data=params, headers=self.headers_list, timeout=(5, 20),proxies=self.proxies)
                ...
                if tr.xpath('./td[@class="name"]/a/@href'):
                    ...
                    try:
                        ...
                        if self.page_items_retry < 3:
                            ...
                            time.sleep(60)
                            self.get_searchsql()
                            page_items = self.get_page_items(page_num, year, subject_info, item_info)
                        else:
                            self.page_items_retry = 0
                            page_items = ''
    ...
    def get_page_list(self):
        ...
        for index,moudle_data in enumerate(moudle_datas[:2]):
            subject_info = moudle_data['subject_info']
            for item_info in moudle_data['origin_info'].values():
                # little_key 为 每个小杂志
                print('当前是 {} 年, 学科为: {}, 杂志为: {}'.format(year, str(subject_info['subject_name']),item_info['origin_name']))
                time.sleep(2)
                if not 'have_spider' in json_data[year][index]['origin_info'][item_info['origin_name']]:
                    print('当前模块未被抓去过, 可放心大胆去抓取!!!')
                    time.sleep(2)
                    for page_num in range(1, 301):
                        print('***' * 20)
                        print('当前是 {} 年, 学科为: {}, 杂志为: {} 中的第 {} 页数据'.format(year, str(subject_info['subject_name']),item_info['origin_name'], page_num))
                        print('***' * 20)
                    ...
                    json_data[year][index]['origin_info'][item_info['origin_name']]['have_spider'] = spider_date
                    new_data = json.dumps(json_data,ensure_ascii=False)
                    with open(self.ori_jsondata_url,'w',encoding='utf-8') as f:
                        f.write(new_data)
                    print('开始进入下一模块...., 请稍后!!')
                else:
                    print('{} 模块已经抓去过, 开始进入下一模块.'.format(item_info['origin_name']))
                    time.sleep(2)
    ...
    def download_detail_pdf(self, page_item_data):
        ...
        for item in page_item_data:
            ...
            if duplicate_status == 1:
                ...
                try:
                    res = self.session.get(detail_url, headers=headers_detail, timeout=(10, 20),proxies=self.proxies)
    ...
    def download_pdf(self, item):
        ...
        try:
            ...
            res = self.session.get(item['pdf_url'], headers=headers_pdf, timeout=(10, 20),proxies=self.proxies)
            time.sleep(random.uniform(0,0.5))
    ...
    def save(self, item):
        ...
        item['origin_moudle'] = '单位_医院_学科_文献来源_{}'.format(self.year)
