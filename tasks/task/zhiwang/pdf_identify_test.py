                ...
                #     text = page.extract_text()
                    if text:
                        ...
                        if re.findall('(作者.*?:.*)',data):
                            item.extend(re.findall('(作者.*?:.*\n.*)',data))
                        if re.findall('1[356789]\d{9}\d{0,3}', data):
                            print('疑似手机号为: ', re.findall('1[356789]\d{9}\d{0,3}', data))
                            item.extend(re.findall('1[356789]\d{9}\d{0,3}', data))
