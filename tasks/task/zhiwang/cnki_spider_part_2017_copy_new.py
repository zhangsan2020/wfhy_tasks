...
from .zw_common import user_info, headers_list, choice_moudle, min_limit
...
class ZwSpider():
    ...
    def get_page_items(self,page_num,year,subject_info,study_info):
        ...
        # self.queryjson = '{"Platform":"","DBCode":"CFLS","KuaKuCode":"CJFQ,CDMD,CIPD,CCND,BDZK,CISD,SNAD,CCJD,GXDB_SECTION,CJFN,CCVD","QNode":{"QGroup":[{"Key":"Subject","Title":"","Logic":1,"Items":[{"Title":"'+ self.searchinfo['model'] +'","Name":"' + self.searchinfo['model_match_data'] + '","Value":"' + self.searchinfo['keywords'] + '","Operate":"%"}],"ChildItems":[]},{"Key":"SCDBGroup","Title":"","Logic":1,"Items":[],"ChildItems":[{"Key":"2","Title":"","Logic":1,"Items":[{"Key":"'+ subject_info['value'] +'?","Title":"' + subject_info['subject_name'] + '","Logic":2,"Name":"' + subject_info['field'] + '","Operate":"","Value":"' + subject_info['value'] + '?","ExtendType":14,"ExtendValue":"","Value2":"","BlurType":""}],"ChildItems":[]},{"Key":"3","Title":"","Logic":1,"Items":[{"Key":"' + year + '","Title":"' + year + '","Logic":2,"Name":"年","Operate":"","Value":"' + year + '","ExtendType":0,"ExtendValue":"","Value2":"","BlurType":""}],"ChildItems":[]},{"Key":"4","Title":"","Logic":1,"Items":[{"Key":"' + study_info['value'] + '","Title":"' + study_info['study_name'] + '","Logic":2,"Name":"' + study_info['study_level_field'] + '","Operate":"","Value":"' + study_info['value'] + '","ExtendType":14,"ExtendValue":"","Value2":"","BlurType":""}],"ChildItems":[]}]}]}}'


        self.queryjson = '{"Platform":"","DBCode":"CFLQ","KuaKuCode":"CJFQ,CDMD,CIPD,CCND,BDZK,CISD,SNAD,CCJD,GXDB_SECTION,CJFN,CCVD","QNode":{"QGroup":[{"Key":"Subject","Title":"","Logic":1,"Items":[{"Title":"' + self.searchinfo['model'] + '","Name":"' + self.searchinfo['model_match_data'] + '","Value":"' + \
self.searchinfo['keywords'] + '","Operate":"%"}],"ChildItems":[]},{"Key":"MutiGroup","Title":"","Logic":1,"Items":[],"ChildItems":[{"Key":"2","Title":"","Logic":1,"Items":[{"Key":"' + subject_info['value'] + '?","Title":"' + subject_info['subject_name'] + '","Logic":2,"Name":"' + subject_info['field'] + '","Operate":"","Value":"' + subject_info['value'] + '?","ExtendType":14,"ExtendValue":"","Value2":"","BlurType":""}],"ChildItems":[]},{"Key":"3","Title":"","Logic":1,"Items":[{"Key":"' + year + '","Title":"' + year + '","Logic":2,"Name":"年","Operate":"","Value":"' + year + '","ExtendType":0,"ExtendValue":"","Value2":"","BlurType":""}],"ChildItems":[]},{"Key":"4","Title":"","Logic":1,"Items":[{"Key":"' + study_info['value'] + '","Title":"' + study_info['study_name'] + '","Logic":2,"Name":"' + study_info['study_level_field'] + '","Operate":"","Value":"' + study_info['value'] + '","ExtendType":14,"ExtendValue":"","Value2":"","BlurType":""}],"ChildItems":[]}]}]}}'
        params = {
            'IsSearch': 'true',
            'QueryJson': self.queryjson,
    ...
    def get_page_list(self):
        ...
        # for year in years:
        year = '2021'
        with open('F:/wanfang_tasks/tasks/task/zhiwang/old_单位_医院.json', 'r', encoding='utf-8') as f:
            data = f.read()
        choice_moudle = json.loads(data)[year]
        for subject_name,subject_data in choice_moudle.items():
            subject_info = {}
            subject_info['subject_name'] = subject_name
            subject_info['field'] = subject_data['field']
            subject_info['value'] = subject_data['value']
            print('当前学科为, {}'.format(subject_name))
            print('学科field为: {}, value 为: {}'.format(subject_info['field'],subject_info['value']))
            for study_name,study_info in subject_data['study_level'].items():
                print('这是study_info: {}'.format(study_info))
                study_info['study_name'] = study_name
                for page_num in range(1,301):
                    print('***'*20)
                    print('当前是 {} 年, 学科为: {}, 研究层次为: {} 中的第 {} 页数据'.format(year,str(subject_name),str(study_name),page_num))
                    print('***'*20)
