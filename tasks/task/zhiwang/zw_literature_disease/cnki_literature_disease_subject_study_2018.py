...
class ZwSpider():
    ...
    def __init__(self):
        ...
        self.ori_jsondata_url = './task/zhiwang/moudle_data/old_literature_disease_sub_study.json'
        self.list_url = 'https://kns.cnki.net/kns8/Brief/GetGridTableHtml'
        self.userinfo = user_info
        self.headers_list = headers_list
        # self.searchinfo = {'model': '作者单位', 'keywords': '医学'}
        self.searchinfo = {'model': '参考文献', 'keywords': '病'}
    ...
    def get_page_items(self, page_num, year, subject_info, item_info):
        ...
        self.queryjson = '{"Platform":"","DBCode":"CFLS","KuaKuCode":"CJFQ,CDMD,CIPD,CCND,BDZK,CISD,SNAD,CCJD,GXDB_SECTION,CJFN,CCVD","QNode":{"QGroup":[{"Key":"Subject","Title":"","Logic":1,"Items":[{"Title":"' + self.searchinfo['model'] + '","Name":"' + self.searchinfo['model_match_data'] + '","Value":"' + \
                         self.searchinfo['keywords'] + '","Operate":"%"}],"ChildItems":[]},{"Key":"MutiGroup","Title":"","Logic":1,"Items":[],"ChildItems":[{"Key":"2","Title":"","Logic":1,"Items":[{"Key":"' + \
                         subject_info['value'] + '?","Title":"' + subject_info['subject_name'] + '","Logic":2,"Name":"' + subject_info['field'] + '","Operate":"","Value":"' + subject_info[
                             'value'] + '?","ExtendType":14,"ExtendValue":"","Value2":"","BlurType":""}],"ChildItems":[]},{"Key":"3","Title":"","Logic":1,"Items":[{"Key":"' + year + '","Title":"' + year + '","Logic":2,"Name":"年","Operate":"","Value":"' + year + '","ExtendType":0,"ExtendValue":"","Value2":"","BlurType":""}],"ChildItems":[]},{"Key":"4","Title":"","Logic":1,"Items":[{"Key":"' + \
                         item_info['value'] + '","Title":"' + item_info['study_level_name'] + '","Logic":2,"Name":"' + \
                         item_info['study_level_field'] + '","Operate":"","Value":"' + item_info[
                             'value'] + '","ExtendType":14,"ExtendValue":"","Value2":"","BlurType":""}],"ChildItems":[]}]}]}}'
        params = {
            'IsSearch': 'true',
            'QueryJson': self.queryjson,
    ...
    def get_page_list(self):
        ...
        for index,moudle_data in enumerate(moudle_datas):
            ...
            for item_info in moudle_data['study_level'].values():
                little_key = item_info['study_level_name']
                print('当前是 {} 年, 学科为: {}, 标题为: {}'.format(year, str(subject_info['subject_name']),little_key))
                time.sleep(2)
                # print('这是要看的数据111: ', json_data[year][index]['study_level'][little_key])
                if not 'have_spider' in json_data[year][index]['study_level'][little_key]:
                    print('当前模块未被抓取过, 可放心大胆去抓取!!!')
                    # print('这是要看的数据222: ',json_data[year][index]['study_level'][little_key])
                    time.sleep(2)
                    ...
                    json_data[year][index]['study_level'][little_key]['have_spider'] = spider_date
    ...
    def download_pdf(self, item):
        ...
        if res.headers.get('Content-Length'):
            if res.status_code == 200 and ('本篇支付' not in res.text) and int(res.headers['Content-Length']) > min_limit:
                ...
                print('**'*20)
                if self.cookie_update == 1:
                    print('发现有cookie更改过的痕迹, 接下来把更新的cookie放入redis中!!')
                    cookies = self.session.cookies.get_dict()
                    # cookie_date = datetime.now().strftime('%Y%m%d%H%M')
                    cookies['cookie_date'] = datetime.now().strftime('%Y%m%d%H%M')
                    print('这是添加日期后的cookie: ',cookies)
                    print('**'*20)
                    key = os.path.basename(__file__) + '_cookie'
                    self.zw_cookie.insert_cookie(key,cookies)
                ...
                if self.pay_permission > 15:
                    print('检测到连续含有本篇支付或者无权限pdf文件超过10次, 暂停1min, 重新登录获取文件数据!!')
                    time.sleep(60)
                    self.get_searchsql()
                    self.pay_permission = 0
    ...
    def save(self, item):
        ...
        # self.searchinfo = {'model': '参考文献', 'keywords': '医学'}
        item['origin_moudle'] = '{}_{}_学科_研究层次_{}'.format(self.searchinfo['model'],self.searchinfo['keywords'],self.year)
