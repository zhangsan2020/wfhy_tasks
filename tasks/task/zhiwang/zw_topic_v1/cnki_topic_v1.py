...
class ZwSpider():
    ...
    def get_searchsql(self):
        ...
        for i in range(self.retry_down_list_max):
            try:
                ...
                print('第 {} 次获取searchsql出现问题, 休息一下, 继续获取!!'.format(i))
                time.sleep(60)
                continue
            ...
            try:
                ...
                time.sleep(10)
                res = self.session.get(field_json_url, headers=header_field)
            self.model_match_data = re.findall(r'Text:"' + self.search_info['platform'] + '",Value:"(\w+).*?"}', res.text)[0]
            self.queryjson = '{"Platform":"","DBCode":"CFLS","KuaKuCode":"CJFQ,CCND,CIPD,CDMD,BDZK,CISD,SNAD,CCJD,CJFN,CCVD","QNode":{"QGroup":[{"Key":"Subject","Title":"","Logic":1,"Items":[{"Title":"' + self.search_info['platform'] + '","Name":"' + self.model_match_data + '","Value":"' + self.search_info['keywords'] + '","Operate":"%"}],"ChildItems":[]}]}}'
            data = {
                'IsSearch': 'true',
            ...
            try:
                ...
                print('searchsql中获取列表页数据出现异常, 休息一下, 继续抓取!!')
                time.sleep(60)
                continue
            if res.status_code == 200 and ('题名' in res.text and '发表时间' in res.text):
                ...
                if searchsql_data:
                    self.searchsql = searchsql_data[0]
                    self.retry_getsearchsql = 0
                    # print('这是首次获取列表页得到的数据: ',res.text)
                    # with open('zw_first_list_data.html','w',encoding='utf-8') as f:
                    #     f.write(res.text)
                    #     f.close()
                    break
            else:
                ...
                if tr.xpath('./td[@class="name"]/a/@href'):
                    ...
                    try:
                        ...
                        if self.page_self.items_retry < 3:
                            ...
                            time.sleep(60)
                            self.get_searchsql()
                            page_items = self.get_page_self.items(page_num, year, self.item_data)
                        else:
                ...
                for index,self.item_data in enumerate(self.items):
                    ...
                    if not 'have_spider_year' in self.item_data:
                        ...
                        for page_num in range(1, 301):
                            ...
                            page_items = self.get_page_items(page_num, year, self.item_data)
    ...
    def download_detail_pdf(self, page_item_data):
        ...
        if self.pdfdown_fail > 4:
            ...
            self.get_searchsql()
            self.pdfdown_fail = 0
        for self.item in page_item_data:
            # 1代表请求成功
