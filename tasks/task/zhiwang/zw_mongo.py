...
class ZwMongo(MongoStore):

    def find_zw_page_num(self, origin_module):

        print('获取mongo中页码数据的基本条件为: {}'.format(origin_module))
        page_min_data = [page_num['cur_page'] for page_num in self.col.find(
            {'origin_module': origin_module}).sort("cur_page").limit(1)]
        page_max_data = [page_num['cur_page'] for page_num in self.col.find(
            {'origin_module': origin_module}).sort("cur_page", -1).limit(1)]
        # for page_num in page_num_data
        # print('mongo中当前已经存储的页码最小值为: ', page_num_data)

        if page_max_data and page_min_data:
            #     至少有一条数据且能获取到最大最小值
            cur_moudle_sql_total = self.col.count_documents({'origin_module': origin_module})
